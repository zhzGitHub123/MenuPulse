...
两行等宽字体紧凑排布，只占一小块菜单栏空间。没有主窗口，没有 Dock 图标，装上就忘掉它的存在。常驻内存 **13 MB**，可执行文件 **343 KB**，采样时 CPU 占用低于**单核的 1%**。

## 特性

- **极致轻量** — 常驻 13 MB，零第三方依赖，无 SwiftUI 运行时，无渲染循环
- **双指标一屏** — 上传/下载速度与 CPU 占用同时显示，无需点开任何面板
- **自动省电** — 屏幕休眠、用户会话切换、系统睡眠时自动暂停采样，唤醒后恢复
- **闲置节流** — 网络静默且 CPU 空闲时自动拉长采样间隔，减少常驻唤醒
- **自适应外观** — 文本以 `labelColor` 绘制，自动跟随深色/浅色菜单栏
- **宽度恒定** — 两列宽度只测量一次，状态项永不改变尺寸，也不会挤动旁边的菜单栏图标
...
| | 实测 |
| --- | --- |
| 内存占用 | **13 MB**（`phys_footprint`，峰值与稳态相同） |
| CPU 占用（机器活跃时） | **约 0.31%** 单核，300 秒窗口，期间有真实网络流量 |
| — 其中应用自身采样 | **0.033%** —— 读一次计数器只要 0.7 ms |
| — 其中菜单栏刷新 | 其余部分，macOS 为「往菜单栏里放新东西」每次收取约 **8 ms** |
| 可执行文件体积 | **343 KB** |
| 第三方依赖 | **0** |

第二段拆解才是这张表里最诚实的部分：**约 90% 的开销不是我们的代码**，而是菜单栏每次变化时 macOS 收取的费用（Core Animation 图层提交，以及发往 ControlCenter 的一次 XPC 往返）。你可以自己验证：`sample MenuPulse 60`，看主线程的非空闲帧落在哪里。

这个数字也决定了优化策略。采样路径已经只占十分之一，再怎么抠都没有意义；真正能撼动总量的只有两个杠杆：**让每次刷新更便宜**，以及**少刷新几次**。两者都是实测出来的，不是猜的——数字从 0.633% 分阶段降到今天：
...
| 状态项定宽（消除菜单栏重排） | 0.567% |
| 显著性阈值 + 3 秒采样间隔 | 0.470% |
| 自定义 `NSView` 重绘替代重建 `NSImage` | **0.313%** |
| *（对照）* 关闭 UI 更新只采样 | *0.033%* |

最后一行是天花板：只要还要在菜单栏上画东西，就绕不过一次图层提交。机器闲下来时，阈值会抑制掉大部分更新、间隔也拉长到 8 秒，实际数字会明显低于上表。

这些数字来自以下取舍：

- **不引入 SwiftUI 运行时。** SwiftUI 的 `MenuBarExtra` 会让一整套渲染管线在应用生命周期内常驻。MenuPulse 是纯 AppKit + 单个 `NSStatusItem`，没有视图树要 diff，也没有渲染循环在跑。
- **刷新路径上没有 `NSImage`。** 状态项里放的是一个自绘两列内容的普通 `NSView`，刷新只是一次 `needsDisplay = true`，而不是重新造一张图交给 `NSStatusBarButton`。绕开 `NSImage` 也就同时绕开了它挑选表示的那套机制，以及 AppKit 每次绘制时对 template 图像的重新着色。仅这一项就占了总降幅的三分之一。
- **主线程不做重活。** 采样跑在独立的 `utility` QoS 队列上，主线程自始至终只做两件事：存下新字符串、给视图打上重绘标记。
- **合并唤醒。** 定时器带 600ms 的 leeway，让内核把这次唤醒和它本来就要做的工作合并处理。周期性应用的能耗大头正是唤醒次数——一个刚性定时器每个周期都会强制产生一次独立唤醒。
